...
public class SecuringAppliancesManager : MonoBehaviour
{
    ...
    [SerializeField] private List<DialogueLineData> introDialogueRich;
    [SerializeField] private List<DialogueLineData> correctAnswerDialogueRich;
    ...
    private void StartMissionFlow()
    {
        ...
        if (dialogueManager != null && introDialogueRich != null && introDialogueRich.Count > 0)
        {
            dialogueManager.ShowDialogueSequence(introDialogueRich, ShowStartQuizGate);
        }
        else
        {
            if (dialogueManager == null)
            {
                Debug.LogWarning("SecuringAppliancesManager: ProdDialogueManager not found; skipping intro dialogue.");
            }
            else
            {
                Debug.LogWarning("SecuringAppliancesManager: introDialogueRich is empty; skipping intro dialogue.");
            }
            ShowStartQuizGate();
        }
    ...
    {
        var dialogueManager = ProdDialogueManager.Instance;
        if (dialogueManager != null && correctAnswerDialogueRich != null && correctAnswerDialogueRich.Count > 0)
        {
            dialogueManager.ShowDialogueSequence(correctAnswerDialogueRich, CompleteStartGate);
            return;
        }

        if (dialogueManager == null)
        {
            Debug.LogWarning("SecuringAppliancesManager: ProdDialogueManager not found; skipping correct-answer dialogue.");
        }
        else
        {
            Debug.LogWarning("SecuringAppliancesManager: correctAnswerDialogueRich is empty; skipping correct-answer dialogue.");
        }
    ...
    private void CompleteMission()
    {
        ...
        {
            // Support placeholders like {securedCount} and {applianceCount} in asset-authored text
            var placeholders = new System.Collections.Generic.Dictionary<string, string>
            {
                {"{securedCount}", securedCount.ToString()},
                {"{applianceCount}", appliances != null ? appliances.Count.ToString() : "0"}
            };

            dialogueManager.ShowDialogueSequence(completionDialogueRich, () =>
            {
                if (statusText != null)
        ...
        else
        {
            if (dialogueManager == null)
            {
                Debug.LogWarning("SecuringAppliancesManager: ProdDialogueManager not found; skipping completion dialogue.");
            }
            else
            {
                Debug.LogWarning("SecuringAppliancesManager: completionDialogueRich is empty; skipping completion dialogue.");
            }
