...
/// Handles timer, flood line, appliance validation, dialogue, and scoring.
/// </summary>
public class SecuringAppliancesManager : MonoBehaviour
{
    ...
    [Header("Scoring")]
    [SerializeField] private int baseScorePerAppliance = 100;
    [SerializeField] private int penaltyPerIllegalMove = 10;
    ...
    private void StartMissionFlow()
    {
        ...
        if (ProdDialogueManager.Instance != null)
        {
            var lines = new List<ProdDialogueLine>
            {
                ...
                new ProdDialogueLine("Professor Lingap", "Timer is for score only—move quickly and safely."),
            };
    ...
    private void CompleteMission()
    {
        ...
        float timeRatio = Mathf.Clamp01(1f - (timeRemaining / missionTimeSeconds));
        int score = securedCount * baseScorePerAppliance - illegalMoves * penaltyPerIllegalMove;
        score = Mathf.Max(0, score);
        string grade = ComputeGrade(score, timeRatio);
        ...
        if (ProdDialogueManager.Instance != null)
        {
            var lines = new List<ProdDialogueLine>
            {
                ...
                new ProdDialogueLine("Professor Lingap", $"Score: {score} | Grade: {grade}"),
    ...
    private string ComputeGrade(int score, float timeRatio)
    {
        // Simple grading: A/B/C based on penalties and speed
        if (score >= baseScorePerAppliance * appliances.Count - penaltyPerIllegalMove * 1 && timeRatio < 0.7f)
            return "A";
        if (score >= baseScorePerAppliance * appliances.Count * 0.7f)
            return "B";
        return "C";
    }
